...
export default function MobileJoystick() {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const moveX = Math.cos(angle) * (clampedDist / maxRadius);
    const moveY = Math.sin(angle) * (clampedDist / maxRadius);

    thumbRef.current.style.transform = `translate(${Math.cos(angle) * clampedDist}px, ${Math.sin(angle) * clampedDist}px)`;

    EventBus.emit('input-move', { x: moveX, y: moveY });
  };
  ...
  return (
    <div
      className="fixed bottom-12 left-12 w-32 h-32 bg-black/40 border-2 border-jade/30 rounded-full pointer-events-auto flex items-center justify-center touch-none"
      ref={joystickRef}
