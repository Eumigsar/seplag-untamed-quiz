...
export default function CharacterCreation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      .from('characters')
      .insert({ user_id: user.id, name, avatar_data })
      .select()
      .single();
              ...
              <div className="flex justify-center gap-2">
                {['light', 'tan', 'dark'].map((s) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setSkin(s)}
                    className={`w-8 h-8 rounded-full border-2 ${skin === s ? 'border-jade' : 'border-transparent'}`}
                    style={{ backgroundColor: s === 'light' ? '#F3E5AB' : s === 'tan' ? '#D2B48C' : '#5C4033' }}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="flex justify-center gap-2">
                {['black', 'brown', 'silver'].map((h) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setHair(h)}
                    className={`w-8 h-8 rounded-full border-2 ${hair === h ? 'border-jade' : 'border-transparent'}`}
