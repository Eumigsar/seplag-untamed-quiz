...
interface ActiveDialog { title: string; text: string; questId?: string; }
...
export default function DialogBox() {
  ...
  useEffect(() => {
    const handleInteract = (data: { name: string; questId?: string }) => {
      setActiveDialog({
        title: data.name,
        text: 'Bem-vindo, estrangeiro. Este mundo é construído por símbolos. Sinta a energia do pincel.',
        questId: data.questId,
      });
    };
    EventBus.on(GAME_EVENTS.INTERACT_NPC, handleInteract);
    return () => { EventBus.off(GAME_EVENTS.INTERACT_NPC, handleInteract); };
  }, []);
  ...
  const closeDialog = () => { setActiveDialog(null); EventBus.emit('dialog-closed'); };

  if (!activeDialog) return null;

  return (
    <div className="fixed inset-x-0 bottom-10 flex justify-center items-end px-4">
      <div className="w-full max-w-2xl bg-black/95 border-2 border-jade/50 rounded-xl p-6 shadow-[0_0_20px_rgba(0,168,107,0.3)] pointer-events-auto">
        ...
        <div className="grid grid-cols-5 gap-3 mb-8">
          {INITIAL_HANZI.map((h) => (
            <button key={h.hanzi} onClick={() => handleLearn(h)} className="group bg-jade/5 border border-jade/20 p-3 rounded-lg hover:bg-jade/20 hover:border-jade transition-all">
              <span className="text-3xl block mb-1">{h.hanzi}</span>
              <span className="text-[10px] text-paper/40 uppercase block">{h.pinyin}</span>
            </button>
          ))}
        </div>
        <button onClick={closeDialog} className="w-full py-3 bg-jade/20 hover:bg-jade/40 border border-jade text-jade font-bold rounded-lg transition-colors">
